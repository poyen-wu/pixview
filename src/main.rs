...
fn get_video_duration(path: &Path) -> Result<f64> {
    ...
    if !output.status.success() {
        bail!(
            "ffprobe failed (is ffmpeg installed?): {}",
...
fn extract_frame(path: &Path, timestamp: f64) -> Result<DynamicImage> {
    ...
    if output.stdout.is_empty() {
        bail!("ffmpeg produced no output at timestamp {}", timestamp);
    }
...
fn show_video<W: Write>(
    ...
    let first = extract_frame(path, timestamps[0])?;
...

    {
        ...
        std::thread::spawn(move || {
            let tmp = std::env::temp_dir().join("pixview_frames");
            ...
            for batch in 0..10 {
                ...
                let mut out_paths: Vec<(usize, std::path::PathBuf)> = Vec::new();

                for (input_idx, &frame_idx) in batch_indices.iter().enumerate() {
                    let ts = timestamps_clone[frame_idx];
                    args.extend([
                        "-an".into(),
                        "-ss".into(),
                        ts.to_string(),
                    ...
                    let out_path = tmp.join(format!("b{}_f{}.png", batch, frame_idx));
                    out_paths.push((frame_idx, out_path.clone()));
                    args.extend([
                        "-map".into(),
                        format!("{}:v", input_idx),
                        "-frames:v".into(),
                ...
                loop {
                    ...
                    for &(frame_idx, ref out_path) in &out_paths {
                        if loaded[frame_idx] {
                            continue;
                        }
                        if out_path.exists() {
                            if let Ok(img) = image::open(out_path) {
                                frames.lock().unwrap()[frame_idx] = Some(img);
                                loaded[frame_idx] = true;
                                loaded_count += 1;
                                batch_loaded += 1;
                            }
                        }
                    ...
                    if batch_loaded == batch_len {
                        break;
                    }

                    let ffmpeg_done = if let Some(ref mut c) = child {
                    ...
                    if ffmpeg_done {
                        for &(frame_idx, ref out_path) in &out_paths {
                            if !loaded[frame_idx] && out_path.exists() {
                                if let Ok(img) = image::open(out_path) {
                                    frames.lock().unwrap()[frame_idx] = Some(img);
                                    loaded[frame_idx] = true;
                                    loaded_count += 1;
                ...
                Event::Key(key) => match key.code {
                    ...
                    KeyCode::Backspace | KeyCode::Left | KeyCode::Char('h') => {
                        if let Some(parent) = cwd.parent() {
                            ...
                            // Reload to immediately find the old directory name
                            entries = load_entries(&cwd);
                            selected = entries
                                .iter()
                            ...
                            scroll = 0;
                            needs_refresh = false; // We already handled loading
                        }
                    }
                    KeyCode::Char('c') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                        return Ok(())
