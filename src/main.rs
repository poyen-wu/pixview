...
fn extract_frame(path: &Path, timestamp_sec: f64) -> Result<DynamicImage> {
                                        ...
                                        while decoder.receive_frame(&mut decoded).is_ok() {
                                            ...
                                            if let Some(scaler) = scaler_opt.as_mut() {
                                                if scaler.run(&decoded, &mut rgb_frame).is_ok() {
                                                    ...
                                                    frames.lock().unwrap()[i] = Some(DynamicImage::ImageRgb8(img_buf));
                                                    loaded[i] = true;
                                                    loaded_count += 1;
