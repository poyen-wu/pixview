...
pub fn read_entry(path: &EntryPath) -> Result<Vec<u8>> {
    match path {
        ...
        EntryPath::InSevenZ(arc, name) => read_7z_entry(arc, name),
    }
}
...
pub fn list_archive(cwd: &EntryPath) -> Result<Vec<ArchiveEntry>> {
    match cwd {
        ...
        EntryPath::InSevenZ(arc, prefix) => list_7z(arc, prefix),
...
pub fn stream_video(path: &EntryPath) -> Option<(String, Arc<AtomicBool>)> {
    match path {
        ...
        EntryPath::InSevenZ(arc, name) => Some(stream_7z_video(arc.clone(), name.clone())),
    }
}
...
fn stream_7z_video(archive: PathBuf, name: String) -> (String, Arc<AtomicBool>) {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let port = listener.local_addr().unwrap().port();
    let stop_signal = Arc::new(AtomicBool::new(false));
    let stop_clone = Arc::clone(&stop_signal);
